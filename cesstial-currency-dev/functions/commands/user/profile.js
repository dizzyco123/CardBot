...
if (!Data) return lib.discord.interactions['@1.0.1'].responses.update({
  token,
  content: `User has not yet played.`,
});
  ...
  embeds: [
    {
      ...
      description: [
        ...
        `**• Coins**: ${userData.Cash}`,
        `**• Bank**: ${userData.Bank}`,
        `**• Total**: ${userData.Networth}`,
        `**• Created Transactions**: ${userData.Created_Transactions.length || 'No created transactions'}`,
        `**• Received Transactions**: ${userData.Received_Transactions.length || 'No receieved transactions'}`,
        `**• Items**: ${Items.length ? Items.join(', ') : 'No items'}`,
      ].join('\n'),
